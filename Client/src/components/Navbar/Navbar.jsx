  ...
  return (
    <>
      ...
      <nav className={styles.bottomNavbar}>
        {/* Corner trims for bottom navbar */}
        <div className={`${styles.cornerTrim} ${styles.bottomNavbarTopLeft}`} />
        <div className={`${styles.cornerTrim} ${styles.bottomNavbarTopRight}`} />
        <div className={`${styles.cornerTrim} ${styles.bottomNavbarBottomLeft}`} />
        <div className={`${styles.cornerTrim} ${styles.bottomNavbarBottomRight}`} />
        ...
        <div className={styles.bottomNavbarInner}>
          {navLinks.map((link) => (
            <button
              key={link.id}
              onClick={() => handleNavClick(link.id)}
              className={`${styles.bottomNavItem} ${activeSection === link.id ? styles.bottomActive : ''} interactive`}
              aria-label={link.label}
            >
              <link.icon size={20} className={styles.bottomNavIcon} />
              {activeSection === link.id && (
                <motion.span
                  layoutId="activeBottomPill"
                  className={styles.bottomActiveIndicator}
                  transition={{ type: "spring", stiffness: 350, damping: 28 }}
                />
              )}
            </button>
